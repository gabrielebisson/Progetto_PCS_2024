                                                      ...
                                                      const Eigen::VectorXi& materials) const
{
    ...
    for (unsigned int l = 0; l < polygons_vertices.size(); l++)
    {
        ...
        {
            // Trasformazione del poligono con 5 vertici in triangoli
            for (size_t i = 2; i < polygons_vertices[l].size(); ++i)
            {
                cells.push_back(UCDCell(UCDCell::Types::Triangle,
                                        { polygons_vertices[l][0], polygons_vertices[l][i-1], polygons_vertices[l][i] },
                                        materials.size() == static_cast<unsigned int>(polygons_vertices.size()) ?
                                            materials[l] :
                                            0));
            }
            continue; // Salta il push_back del poligono originale
        }
        else
